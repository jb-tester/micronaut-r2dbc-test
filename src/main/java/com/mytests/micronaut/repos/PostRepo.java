...
public interface PostRepo extends ReactiveStreamsCrudRepository<Post, Integer> {
    ...
    @SingleResult @Join(value = "author", alias = "auth_")
    Publisher<Post> findFirstByVersion(Integer version);
    //Single<Post> findFirstByVersion(Integer version);  // Single<> type is incorrect here

    @Query("select p.title from posts p where p.status = 'DRAFT'")
    Publisher<String> findDrafts();
    //Flowable<String> findDrafts();  // Flowable<> type is incorrect here
    ...
    @SingleResult
    @Join(value = "author", type = Join.Type.FETCH)
    Publisher<Post> findFirstOrderByIdDesc();  // not parsed
